...
#[derive(Parser, Debug)]
#[command(author, version, about = "Files index CLI (Практична робота 2)", long_about = None)]
struct Cli {
    #[command(subcommand)]
    cmd: Commands,
}
...
/// Run practical work 2 CLI using the provided argv (first element should be program name).
pub fn run_from_args(argv: Vec<String>) -> Result<()> {
...
// Embedded store implementation to keep the module self-contained
mod store {
